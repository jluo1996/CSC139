...
int main(int argc, char *argv[]){
	...
	for(i = 0; i < gThreadCount; i++){
		pthread_attr_init(&attr[i]);	// get the default attributes
	...
	while(!termination){
		...
		for(i = 0; i < gThreadCount; i++){
			if(gThreadDone[i] == true){
				if(gThreadProd[i] == 0){
					i = gThreadCount + 99;	// to exit for loop
					termination = true;	// to exit while loop
				}
				else{
					gDoneThreadCount++;
				}
		...
		// if number of done threads are equal to thread count after for loop, means all threads are done
		if(gDoneThreadCount == gThreadCount){
			termination = true;	// exit the while loop
		}
	}
	
	// cancel all threads since we found a zero
	for(i = 0; i < gThreadCount; i++){
	...
	sem_init(&mutex, 0, 1);
	sem_init(&completed, 0, 0);		// initialize it to 0
	SetTime();
	...
	sem_wait(&completed);	// acquire "completed" semaphore
...
int SqFindProd(int size) {
	...
	int i;
	for(i = 1; i < size; i++){
		// if find a zero, product must be 0
		if(gData[i] == 0){
			return 0;
		}
		else{
...
void* ThFindProd(void *param) {
	...
	int start = ((int*)param)[1];		// carry the start index
	int end = ((int*)param)[2];		// carry the  end index
	
	int i;
	for(i = start; i < (end + 1); i++){
		if(gData[i] == 0){	// if find a zero, product is 0 and exit
...
void* ThFindProdWithSemaphore(void *param) {
	...
	for(i = start; i < (end + 1); i++){
		if(gData[i] == 0){
			gThreadProd[threadNum] = 0;
			sem_post(&completed);	// release "completed" semaphore
			break;
		}
		else{
			gThreadProd[threadNum] *= gData[i];
	...
	gDoneThreadCount++;
	if(gDoneThreadCount == gThreadCount){
		sem_post(&completed);	// if it's the last thread, release "completed" semaphore
	}
...
void CalculateIndices(int arraySize, int thrdCnt, int indices[MAX_THREADS][3]) {
	...
	indices[thrdCnt - 1][2] = arraySize - 1;	// last thread handles all the remainder indices
}
