...
int main(int argc, char* argv[])
{
	...
	if (pid < 0) { /* error occurred */
		...
		// have a loop executed itemCnt time 
...
void InitShm(int bufSize, int itemCnt)
{
    int in = 0;
    int out = 0;
	const char *name = "OS_HW1_JianliangLuo"; // Name of shared memory object to be passed to shm_open
	...
	ftruncate(shm_fd, SHM_SIZE); 		// SHM_SIZE = 4096
    ...
    SetBufSize(bufSize); 	// buff size
   	SetItemCnt(itemCnt);	// item count
	SetIn(in);		// in
	SetOut(out);		// out
...
{   
	...
	for(i = 0; i < GetItemCnt(); i++){
		while(((GetIn() + 1) % GetBufSize()) == GetOut())
			;	// do nothing
		int val = GetRand(0,2500);		// generate a random number at range [0-100]
		WriteAtBufIndex(GetIn(), val);		// write the random number into buf[in]
		// report the production of an item:
		printf("Producing Item %d with value %d at Index %d\n", i, val, GetIn());
		SetIn((GetIn() + 1) % GetBufSize());	// update in
	}
